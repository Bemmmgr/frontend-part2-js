...
class App {
  ...
  _loadMap(position) {
    ...
    this.#map = L.map('map').setView(coords, this.#mapZoomLevel); // 15 for zoom scale
    // console.log(map);
  ...
  _renderWorkoutMarker(workout) {
    L.marker(workout.coords)
      ...
      .setPopupContent(
        `${workout.type === 'runing' ? '🏃‍♂️' : '🚴'} ${workout.description}`
      )
      .openPopup();
  }

  // 15013 - rendering workouts
